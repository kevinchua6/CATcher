 ...
 * using GitHub.
 */
export class ClosedIssueService {
  ...
  startPollIssues() {
    if (this.issuesPollSubscription === undefined) {
      ...
      this.issuesPollSubscription = timer(0, ClosedIssueService.POLL_INTERVAL)
        .pipe(
          exhaustMap(() => {
  ...
  pollIssue(issueId: number): Observable<Issue> {
    return timer(0, ClosedIssueService.POLL_INTERVAL).pipe(
      exhaustMap(() => {
      ...
      case 'FILTER_BY_CREATOR':
        issuesAPICallsByFilter.push(
          this.githubService.fetchIssuesGraphql(
            new RestGithubIssueFilter({ creator: this.userService.currentUser.loginId, states: 'close' })
          )
        );
        break;
